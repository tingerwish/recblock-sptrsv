    ...
    for (int re = 0; re < BENCH_REPEAT; re++)
    {
        ...
        for (int i = 0; i < sum_block; i++)
        {
            if (i % 2 == 0)
            {
                ...
                {
                    sptrsv_syncfree_csc_cuda_executor_fasttrack<<<trsv_blk[tri_index].num_blocks, trsv_blk[tri_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                ...
                else if (trsv_blk[tri_index].method == 1)
                {
                    if (sizeof(VALUE_TYPE) == 8)
                        // 调用cusparseDcsrsv2_solve函数，用于求解CSR格式的稀疏矩阵的线性方程组
                    cusparseDcsrsv2_solve(trsv_blk[tri_index].handle, // cusparse句柄
                      trsv_blk[tri_index].trans, // 转置标志
                      trsv_blk[tri_index].m, // 矩阵的行数
                      trsv_blk[tri_index].nnzTR, // 矩阵的非零元素个数
                      &(trsv_blk[tri_index].alpha_double), // 系数
                      trsv_blk[tri_index].descr, // 矩阵描述符
                      (double *)(&d_recblock_Val[index_offset[i]]), // 矩阵的非零元素
                      &d_recblock_Ptr[ptr_offset[i]], // 矩阵的行指针
                      &d_recblock_Index[index_offset[i]], // 矩阵的列索引
                      trsv_blk[tri_index].info, // 矩阵信息
                      (double *)&(b_t[b_offset]), // 输入向量
                      (double *)&(x_t[x_offset]), // 输出向量
                      trsv_blk[tri_index].policy, // 策略
                      trsv_blk[tri_index].pBuffer); // 缓冲区
                    else if (sizeof(VALUE_TYPE) == 4)
                        cusparseScsrsv2_solve(trsv_blk[tri_index].handle, trsv_blk[tri_index].trans, trsv_blk[tri_index].m, trsv_blk[tri_index].nnzTR, 
                        &(trsv_blk[tri_index].alpha_float), trsv_blk[tri_index].descr,
                ...
                else if (trsv_blk[tri_index].method == 2)
                {
                    for (int li = 0; li < trsv_blk[tri_index].nlv; li++)
                    {
                        if (li == 0)
                        {
                            ...
                            sptrsv_levelset_threadsca_csr_cuda_executor_fasttrack<<<trsv_blk[tri_index].num_blocks, trsv_blk[tri_index].num_threads>>>
                            (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                            trsv_blk[tri_index].m_lv_array[li], trsv_blk[tri_index].m, trsv_blk[tri_index].offset_array[li], trsv_blk[tri_index].substitution, 
                            &b_t[b_offset], &x_t[x_offset]);
                        }
                        else
                        {
                            if ((trsv_blk[tri_index].nnz_lv_array[li] / trsv_blk[tri_index].m_lv_array[li]) <= 15)
                            {
                                trsv_blk[tri_index].num_threads = WARP_PER_BLOCK * WARP_SIZE;
                                trsv_blk[tri_index].num_blocks = ceil((double)(trsv_blk[tri_index].m_lv_array[li]) / (double)(trsv_blk[tri_index].num_threads));
                                sptrsv_levelset_threadsca_csr_cuda_executor<<<trsv_blk[tri_index].num_blocks, trsv_blk[tri_index].num_threads>>>
                                (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                                trsv_blk[tri_index].m_lv_array[li], trsv_blk[tri_index].m, trsv_blk[tri_index].offset_array[li], trsv_blk[tri_index].substitution, 
                                &b_t[b_offset], &x_t[x_offset]);
                            }
                            else
                            {
                                ...
                                sptrsv_levelset_warpvec_csr_cuda_executor<<<trsv_blk[tri_index].num_blocks, trsv_blk[tri_index].num_threads>>>
                                (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                                trsv_blk[tri_index].m_lv_array[li], trsv_blk[tri_index].m, trsv_blk[tri_index].offset_array[li], trsv_blk[tri_index].substitution, 
                                &b_t[b_offset], &x_t[x_offset]);
                            }
                ...
                {
                    sptrsv_syncfree_warpvec_csc_cuda_executor<<<trsv_blk[tri_index].num_blocks, trsv_blk[tri_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
            ...
            else
            {
                ...
                {
                    spmv_threadsca_csr_cuda_executor<<<mv_blk[squ_index].num_blocks, mv_blk[squ_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                    mv_blk[squ_index].m, &x_t[loc_off[i]], mv_blk[squ_index].d_y);
                    if (mv_blk[squ_index].longrow != 0)
                        spmv_longrow_csr_cuda_executor<<<mv_blk[squ_index].num_blocks_l, mv_blk[squ_index].num_threads_l>>>
                        (mv_blk[squ_index].d_csrRowPtr_l, mv_blk[squ_index].d_csrColIdx_l, mv_blk[squ_index].d_csrVal_l,
                ...
                {
                    spmv_threadsca_dcsr_cuda_executor<<<mv_blk[squ_index].num_blocks, mv_blk[squ_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                ...
                {
                    spmv_warpvec_csr_cuda_executor<<<mv_blk[squ_index].num_blocks, mv_blk[squ_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                    mv_blk[squ_index].m, &x_t[loc_off[i]], mv_blk[squ_index].d_y);
                    if (mv_blk[squ_index].longrow != 0)
                        spmv_longrow_csr_cuda_executor<<<mv_blk[squ_index].num_blocks_l, mv_blk[squ_index].num_threads_l>>>
                ...
                {
                    spmv_warpvec_dcsr_cuda_executor<<<mv_blk[squ_index].num_blocks, mv_blk[squ_index].num_threads>>>
                    (&d_recblock_Ptr[ptr_offset[i] - 1], &d_recblock_Index[index_offset[i]], &d_recblock_Val[index_offset[i]],
                    mv_blk[squ_index].m_new, &x_t[loc_off[i]], mv_blk[squ_index].d_y, &d_recblock_dcsr_rowidx[dcsrindex_offset[i]]);
                    if (mv_blk[squ_index].longrow != 0)
                        spmv_longrow_csr_cuda_executor<<<mv_blk[squ_index].num_blocks_l, mv_blk[squ_index].num_threads_l>>>
